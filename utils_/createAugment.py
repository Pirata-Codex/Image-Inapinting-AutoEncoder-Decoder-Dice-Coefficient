...
class createAugment(keras.utils.Sequence):
    ...
    def __data_generation(self, idxs):
        ...
        Masked_images = np.empty((self.batch_size, self.dim[0], self.dim[1],
                                  self.n_channels))  # Masked image
        # Mask_batch is a matrix of binary masks used as input
        Mask_batch = np.empty((self.batch_size, self.dim[0], self.dim[1],
                               self.n_channels))  # Binary Masks
        # y_batch is a matrix of original images used for computing error from reconstructed image
        y_batch = np.empty((self.batch_size, self.dim[0], self.dim[1],
                            self.n_channels))  # Original image
    ...
    def __createMask(self, img):
        ...
        mask = np.full((size, size, channels), 255, np.uint8)  ## White background
